...
export class BookingPage {
    static async viewBookingPage(page) {
        try {
            const reviewItineraryMessage = await page.textContent(bookingpageSelectors.reviewItinerary);

            expect(reviewItineraryMessage).toBe(ITINERARY_MESSAGE);

            logger.info(`Review itinerary message verified: "${ITINERARY_MESSAGE}"`);
        } catch (error) {
