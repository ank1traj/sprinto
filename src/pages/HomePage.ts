...
import { ONE_WAY, ROUND_TRIP, CLASS_TYPES, NO_ARRIVAL_ERROR_MESSAGE, NO_DEPARTURE_ERROR_MESSAGE, NO_ARRIVAL_DEPARTURE_ERROR_MESSAGE } from '@common/constants'
...
export class HomePage {
    ...
    static async searchWhereFrom(page, whereFrom, whereFromAirport) {
        ...
        const allFrom = await page.locator(homepageSelectors.dropdownSelector).textContent()
        ...
        if (airportEntries) {
            for (const entry of airportEntries) {
                if (entry.includes(whereFromAirport)) {
                    ...
                    logger.info("Successfully identified the desired departure airport.");
                    break;
                }
            }
        }
        ...
        if (airportEntries) {
            for (const entry of airportEntries) {
                if (entry.includes(whereToAirport)) {
                    returnAirport = entry;
                    logger.info("Successfully identified the desired return airport.");
                    break;
                }
            }
        }
    ...
    static async selectDepartureDate(page, desiredDate?: string) {
        ...
        if (desiredDate) {
            ...
            desiredDepartureMonth = dateObject.toLocaleString('en-US', { month: 'long', year: 'numeric' });
            desiredDepartureDay = dateObject.toLocaleString('en-US', { month: 'short', day: 'numeric' });
        ...
        currentMonth = await page.locator(homepageSelectors.departureMonth).textContent();
    ...
    static async selectReturnDate(page, desiredDate?: string) {
        ...
        if (desiredDate) {
            const dateObject = new Date(desiredDate);
            desiredReturnMonth = dateObject.toLocaleString('en-US', { month: 'long', year: 'numeric' });
            desiredReturnDay = dateObject.toLocaleString('en-US', { month: 'short', day: 'numeric' });
    ...
    static async changeTravelWay(page, desiredWay) {
        const currentWay = await page.locator(homepageSelectors.oneWayOrRoundTrip).textContent();
    ...
    static async customizeFlightOptions(page, desiredClassType?:string) {
        ...
        this.changePassengerCount(page, testData.adultsCount, testData.childrenCount, testData.infantsCount)
        let classType;
        if(desiredClassType) {
        ...
        this.getFlightDetailsAfterCustomization(page, testData.adultsCount, testData.childrenCount, testData.infantsCount, classType)
    }

    static async selectTravelClass(page, classText) {
        const travelClassSelector = homepageSelectors.travelClass.replace('${classText}', classText);
    ...
    static async changePassengerCount(page, desiredAdultsCount, desiredChildrenCount, desiredInfantsCount) {
        if(desiredAdultsCount > 9) {
            logger.error("Then number of adults should not exceed 9")
            return;
        }
        if(desiredAdultsCount < desiredInfantsCount) {
            logger.error("The number of adults should be equal to the number of infants");
            return;
        }
        if(desiredAdultsCount + desiredChildrenCount > 9) {
            logger.error("The total number of passengers (adults + children) should not exceed 9");
            return;
        }
        ...
        const adultCountDetails = await page.locator(adultsSelector).textContent();
        const childrenCountDetails = await page.locator(childrenSelector).textContent();
        const infantsCountDetails = await page.locator(infantsSelector).textContent();
        ...
        while (desiredAdultsCount !== adultsCount) {
            if (desiredAdultsCount > adultsCount) {
                await page.locator(homepageSelectors.adultsCountIncrease).click();
            } else {
                await page.locator(homepageSelectors.adultsCountDecrease).click();
            }

            const updatedDetails = await page.locator(adultsSelector).textContent();
        ...
        while (desiredChildrenCount !== childrenCount) {
            if (desiredChildrenCount > childrenCount) {
                await page.locator(homepageSelectors.childrenCountIncrease).click();
            } else {
                await page.locator(homepageSelectors.childrenCountDecrease).click();
            }

            const updatedDetails = await page.locator(childrenSelector).textContent();
            const updatedMatch = updatedDetails.match(regex);
            if (!updatedMatch) {
                logger.error("Unable to determine the updated count");
                return;
        ...
        while (desiredInfantsCount !== infantsCount) {
            if (desiredInfantsCount > infantsCount) {
                await page.locator(homepageSelectors.infantsCountIncrease).click();
            } else {
                await page.locator(homepageSelectors.infantsCountDecrease).click();
            }

            const updatedDetails = await page.locator(infantsSelector).textContent();
            const updatedMatch = updatedDetails.match(regex);
            if (!updatedMatch) {
                logger.error("Unable to determine the updated count");
                return;
    ...
    static async getFlightDetailsAfterCustomization(page, desiredAdultsCount, desiredChildrenCount, desiredInfantsCount, desiredClassType) {
        const flightDetails = await page.locator(homepageSelectors.flightCustomizationDetails).textContent()
        const expectedFlightDetails = Helpers.generateExpectedFlightDetailsTemplate(desiredAdultsCount, desiredChildrenCount, desiredInfantsCount, desiredClassType);
    ...
    static async searchFlightWithoutArrival(page) {
        ...
        const expectedMessage = await page.locator(homepageSelectors.searchWithoutArrival).textContent()
        expect(expectedMessage).toBe(NO_ARRIVAL_ERROR_MESSAGE)
    }

    static async searchFlightWithoutDeparture(page) {
        await page.locator(homepageSelectors.searcHButton)
        const expectedMessage = await page.locator(homepageSelectors.searchWithoutDeparture).textContent()
        expect(expectedMessage).toBe(NO_DEPARTURE_ERROR_MESSAGE)
    }

    static async searchFlightWithoutArrivalAndDeparture(page) {
        await page.locator(homepageSelectors.searcHButton)
        const expectedMessage = await page.locator(homepageSelectors.searchWithoutArrivalAndDeparture).textContent()
