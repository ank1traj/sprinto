...
import { NO_FLIGHTS_FOUND_MESSAGE, PLEASE_TRY_AGAIN_MESSAGE } from '@common/constants'
...
export class SearchPage {
    static async checkNoFlightFound(page) {
        try {
            await page.waitForSelector(searchpageSelectors.errorMessage, { timeout: 5000 });
            await page.waitForSelector(searchpageSelectors.tryAgainMessage, { timeout: 5000 });

            const noFlightsMessage = await page.textContent(searchpageSelectors.errorMessage);
            const tryAgainMessage = await page.textContent(searchpageSelectors.tryAgainMessage);
            ...
            logger.error(`Error in checkNoFlightFound function: ${error.message}`);
    ...
    static async getOneWayFlightAndSortAndBookCheapest(page) {
        ...
        await page.waitForSelector(searchpageSelectors.allOneWayFlight, { timeout: 15000 });
        const allFlights = await page.$$(searchpageSelectors.allOneWayFlight);

        let lowestPrice = Number.MAX_VALUE;
        let lowestPriceIndex;

        await Promise.all(allFlights.map(async (flight, i) => {
            const details = await flight.innerText();
            const detailsArray = details.split('\n');
            const priceValue = detailsArray.find(value => value.includes('₹'));

            if (priceValue) {
                const numericPart = priceValue.match(/[0-9,]+/);

                if (numericPart) {
                    const numericValue = parseInt(numericPart[0].replace(/,/g, ''), 10);
