...
test.describe('negative testcase', () => {
    test.only('search flight without arrival and departure', async ({
        page,
    }) => {
        try {
    ...
    test('search flight without arrival', async ({ page }) => {
        try {
            await HomePage.searchWhereFrom(
                page,
                testdata.whereFrom,
                testdata.WhereFromAirport
            )
            await HomePage.searchFlight(page)
    ...
    test('search flight without departure', async ({ page }) => {
        try {
            await HomePage.searchWhereTo(
                page,
                testdata.whereTo,
                testdata.WhereToAirport
            )
            await HomePage.searchFlight(page)
    ...
    test('search flight with round trip without any flights', async ({
        page,
        context,
    }) => {
        try {
            ...
            await HomePage.searchWhereFrom(
                page,
                testdata.whereFrom,
                testdata.wrongWhereFromAirport
            )
            await HomePage.searchWhereTo(
                page,
                testdata.whereTo,
            ...
            if (result.hasNoFlights) {
                logger.info(
                    'No flights available for the selected criteria. Implementing error handling...'
                )
                logger.warn(
                    'Warning: No flights were found for the specified search criteria.'
                )
                logger.warn(
                    'Warning: Please try again with different airports or dates.'
                )
            } else {
                logger.info(
                    'Flights successfully found for the specified search criteria.'
                )
