...
export class Helpers {
    ...
    static getDesiredReturnDay(): string {
        ...
        const returnDate = new Date(departureDate.setDate(departureDate.getDate() + 7));
        const day = returnDate.getDate().toString().padStart(2, '0');
        console.log(day)
        return `${returnDate.toLocaleString('en-US', { month: 'short' })} ${day}`;
    }

    static getRandomClassType(classTypes: any[]): any {
        const randomIndex: number = Math.floor(Math.random() * classTypes.length);
        return classTypes[randomIndex];
    }

    static generateExpectedFlightDetailsTemplate(adultsCount: number, childrenCount: number, infantsCount: number, classType: string): string {
    ...
    static async scrollUntilSelectorFound(page, selector) {
        ...
        await page.waitForSelector(lazyLoadedElementSelector, { timeout: 15000 });

        const lazyLoadedElementText = await page.$eval(lazyLoadedElementSelector, element => element.textContent.trim());
    ...
    static clickLowestPriceButton = async (page, lowestPriceIndex) => {
        if (lowestPriceIndex !== undefined) {
            ...
            logger.info(`Clicked on the button for the lowest price at index ${lowestPriceIndex}`);
        }
    };
}